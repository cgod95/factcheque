      ...
      <div className="grid gap-6 lg:grid-cols-[1.55fr_1fr] lg:items-start">
        ...
        <figure className="min-w-0">
          ...
          >
            ...
            <g fill="none">
              {links.map((link) => {
                const meta = routeStatusMeta[link.status]
                ...
                return (
                  <path
                    ...
                    strokeOpacity={isActive ? 0.85 : 0.34}
                    className="cursor-pointer transition-[stroke-opacity] duration-150 focus:outline-none"
                    tabIndex={0}
                    role="button"
                    aria-pressed={isActive}
                    aria-label={`${route.from} to ${route.to}. ${meta.label}. ${route.note}`}
          ...
          <figcaption className="mt-3 text-xs leading-relaxed text-muted">
            Colour shows what the bill does to each route. Link widths are uniform and
            illustrative — the sourced figures have different bases (cumulative totals, a
            since-2010 total, and a single investigation) and are not directly comparable, so
            they appear as annotations in the panel, not as widths. Hover, tap or focus a route
            for detail.
          </figcaption>
        </figure>
        ...
        <aside className="lg:sticky lg:top-24">
          <div className="rounded-xl border border-line bg-surface p-5">
            <div className="flex items-center gap-2">
              <span
                className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium ${routeStatusMeta[activeRoute.status].chip}`}
              >
                <span
                  className={`h-1.5 w-1.5 rounded-full ${routeStatusMeta[activeRoute.status].dot}`}
                  aria-hidden="true"
                />
                {routeStatusMeta[activeRoute.status].label}
              </span>
            </div>
